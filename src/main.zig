...
///
/// 默认窗口是 `[last_run, now)`；显式 `--last 24h` 时窗口强制改成
/// `[now-24h, now)`，last_run 不参与窗口大小的计算。
///
/// lastrun 的写回是有条件的：只有当强制窗口跟这个群已经扫到的位置接得上
/// （`last_run >= now-24h`）时才前移到 now。接不上就原地不动并打警告——
/// 否则 `[last_run, now-24h)` 这一段会被永久跳过，里面的 💦 撤稿只会被看到
/// 一次。停机几天后随手跑一句 `--last 1h` 确认服务还活着，就属于这种情形。
///
