...
同一条规则的另一半在表情探测那里：被观察成员的某条消息 `get_msg` 连重试都失败时，它挂没挂 ✨/🔥
本轮无从得知，`lastrun` 因此只前移到**最早**那条探测失败的消息，而不是 `now`。下一轮的窗口会重新
覆盖它（连同它的邻居——🔥 链和 💨 的判定都需要上下文，所以是顶住窗口而不是单独重探那一条）。
瞬时抖动下一轮就自愈；万一某条消息永久探不到，窗口逐日变长，触到 7 天上限后会被跳过并告警，
不会无限停滞。日志里的 `reaction probe for message N failed` 就是这个机制被触发的信号。
