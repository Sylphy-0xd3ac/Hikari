...
| `hikari run --last <时长>` | 忽略本次窗口起点的 `lastrun`，强制重扫最近一段时间；支持正整数 `m`/`h`/`d`，最大 `7d`。若强制窗口跟已扫到的位置接不上，扫描照做但 `lastrun` 不前移（见下） |
...
被拆开重复收录；因此它只补进窗口内尚未入库的候选。

`lastrun` 的写回是**有条件**的：只有当强制窗口跟这个群已经扫到的位置接得上时才前移到 `now`。
接不上就原地不动，并打一条指名空隙区间的警告。这条规则不是保守，是必需的——设想停机五天后跑
一句 `hikari run --last 1h` 确认服务还活着：这一小时确实扫了，但那五天没有，如果照样把 `lastrun`
推到现在，那五天就再也不会被任何一次扫描覆盖，里面的 💦 撤稿只会被看到一次，等于永久丢失。
想真正补上停机跨度，用不带 `--last` 的 `hikari run`（自动按 `lastrun` 算窗口，上限 7 天），
或者把 `--last` 的时长开到足以覆盖整个空隙。

同一条规则还有另外三个触发点，都是"这一轮有件事没看清楚，窗口一滑过去就再也没机会了"：

- 被观察成员的某条消息 `get_msg` 连重试都失败时，它挂没挂 ✨/🔥 本轮无从得知。日志里的
  `reaction probe for message N failed` 就是信号。
- 一条 💦 的撤稿目标彻底解析不了时（`reply target N unresolvable`）。这一条比丢语录严重：
  丢语录只是少收一句，丢撤稿是一句本该下架的话继续挂在公开 API 上。
- 翻页没能一路翻到窗口起点时（`history window NOT fully covered`）。这时窗口起点到本轮最老
  一条之间的那一段**完全没被检查过**，`lastrun` 顶回窗口起点，下一轮重新覆盖整段。翻到群历史
  开头（空页、只剩锚点一条）不算这种情况——那是正常收尾，再往前本来就没有消息。
  翻页护栏跟着窗口跨度走：每 24 小时给 200 页（每页 200 条），7 天窗口最多 1400 页；
  日志里 `page guard exhausted (N pages)` 报的就是当轮的实际上限。

这三种情况下 `lastrun` 只前移到**最早**那个出问题的时刻，而不是 `now`，下一轮的窗口会连同上下文
一起重新覆盖（🔥 链、💨、💦 的一跳都需要邻居消息，所以是顶住窗口而不是单独重放那一条）。瞬时
抖动下一轮就自愈；万一某条永久读不到，窗口逐日变长，触到 7 天上限后会被跳过并告警，不会无限停滞。
日志里 `holding lastrun at ... because something in this window could not be read` 是它生效的标志。
