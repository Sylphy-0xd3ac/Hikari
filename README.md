...
别的路径能表达。💤（见下）不是「又一条路径」，它是一道更前置的闸门：路径 1–4 回答「这条消息该不该
被收录」，💤 回答「这个群这一轮要不要落库」。
...
本身是一条路径 2 的 `✨` 触发消息（含引用、除引用外只有一个 `✨`），就**多解析一跳**，撤掉那条 `✨`
引用的消息，然后所有既有规则（含 🔥 链展开）原样适用。只跳一次；跳到的东西解析不出来、或本身不是
一个可作废的目标时，跟改动之前一样什么都不发生。

**💤 今天先不收**：管理员发一条**只有 💤** 的消息（不含引用、没有别的段、trim 之后逐字节等于 `💤`，
`💤💤` / `睡了💤` / `💤 明天见` 都不算），**这个群**这一轮一条都不收录。其它群不受影响；这个群照样扫、
照样发它那条七行合并转发，全部候选如实计进 `skipped`（`Added 0 messages, skipped N messages.`）——
一个安静发不出东西的群，跟一个死掉的服务，在群里必须长得不一样。**💦 撤稿照常执行**（💤 的意思是
「今天别加东西」，不是「忽略撤稿」），**`lastrun` 照常前移**（不前移的话下一次触发会重扫同一个窗口、
看到同一条 💤、再跳过一次，是个自我持续的永久停摆）。反悔了就用 💦 引用那条 💤，这一轮照常收录。
