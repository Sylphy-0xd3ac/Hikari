...
3. **管理员手动收录**：管理员（`ADMIN_QQS` 之一）发一条不含引用、以 `✨` 开头的消息 → 候选是这条消息，
   正文取 `✨` 之后的部分。
4. **🔥 链式收录**：被观察者把一句话拆成好几条连续发，群里除了 ✨ 还额外贴了 🔥（贴 🔥 而不贴 ✨
   不算数）。窗口内（按去重后的 `message_id` 序列算）相邻不超过 3 条消息的、同时带 ✨+🔥 的多条
   被观察者消息会被拼成**一条**语录，按时间顺序用空格连接正文，主键是第一条的 `message_id`；链不
   设长度上限。一旦并链，这些消息不再各自走路径1单独入选，否则会同时收录拆开的碎句和拼好的整句。

四条路径互不冲突时按 `message_id` 去重；被观察者本人若同时是管理员，路径 3 优先于路径 1、路径 4
（详见设计文档 §4.5.1）。
...
这样跨扫描窗口的 💦 也能作废前几天已经收录的语录。💦 引用到 🔥 链上**任意一个成员**（不论是不是
第一条）都会作废**整条链**：存的那份语录会被删除，链的**全部成员**都会被 tombstone，不只是第一条
——这样即便以后 🔥 被撤掉、链散架，幸存的成员也不会因为没被单独 tombstone 过而绕开撤稿、重新以
路径1的身份入库。作废判定（Pass A）先于收录判定（Pass B）执行，且落盘先于收录：同一次扫描里先处理
完所有作废，再处理收录。
...
首次针对真实 NapCat + Redis 联调时按这个顺序走。这里原本有五处 NapCat 线上行为仓库内验证不了，
2026-08-15 首次生产运行 + 三次针对真实 NapCat 的手工探测（一次对 `get_group_msg_history` 用
`count=5` 分别探测不带锚点 / `reverse_order=false` / `reverse_order=true` 三种调用，一次直接对
`get_msg` 探测，一次针对被观察者的两条连续消息核对 🔥 的 `emoji_id`）已经把全部五条坐实，详见下面
小节；仍按下面的方法核对，出问题时照着改代码。
...
4. **✨ 的 `emoji_id` 是 `"10024"`——已确认，由生产数据坐实。** 定义在 `src/napcat.zig` 的
   ...
   `none matched star_emoji_id=10024 or fire_emoji_id=128293: ...` 这一行；若打出了，冒号后面是
   这条消息上实际出现过的全部 `emoji_id`（`id×次数`），✨ 的真实 id 会在那串里。

5. **🔥 的 `emoji_id` 是 `"128293"`——已确认，由生产数据坐实。** 定义在 `src/napcat.zig` 的
   `fire_emoji_id` 常量。2026-08-15 针对生产 NapCat 探测到两条真实消息 `2015349404`（"你们有钱"）
   与 `283613043`（"你们潇洒"），二者的 `emoji_likes_list` 都同时带 `emoji_id ∈ {"10024", "128293"}`，
   与同一套"Unicode 表情回应用十进制码点做 emoji_id"的编码方案一致（128293 = 0x1F525，即 🔥 = U+1F525
   的十进制形式），核对方法与上一条相同。
