...
**探针失败会顶住 `lastrun`。** 被观察成员的某条消息在 4.4 节的表情探测里 `get_msg` 连同唯一一次
重试都失败时，这条消息挂没挂 ✨/🔥 本轮无从得知；若照常把 `lastrun` 推到 `now`，下一轮的窗口从它
后面开始，这条语录就永远不会再被看到——一次网络抖动换一条永久丢失。因此本轮记下**最早**一条探针
失败消息的 `time`，`lastrun` 只推进到那里（`commitAfterProbeHold`），下一轮窗口自然重新覆盖它。

顶住窗口而不是单独把这个 `message_id` 排进重探队列，是因为 🔥 链的归并（4.4 节）与 💨 的补丁判定
都依赖这条消息在窗口里的**邻居**，脱离上下文单独重探会改变分类语义。最坏情况有界且自愈：某条消息
永久探不到时窗口起点钉在它那里逐日变长，到 4.1 节的 7 天上限被 clamp 跳过并告警，届时恢复正常；
这期间收录照常进行。只探 💤 的非观察成员不参与顶窗口，理由见 4.5.2 节"核验失败 = 未确认"。

稳态窗口长度约等于「每轮双重失败数 + 1」天（失败位置在窗口内近似均匀时，最早那条落在 1/(k+1)
处），k=6 才会撞到 7 天上限。runner 在顶住窗口时打一条指名回退秒数的 warn，运维据此判断 k。
