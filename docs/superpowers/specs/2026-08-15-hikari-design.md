...
**💦 的一跳（2026-08-16 新增）**：`R` 解析出来之后、套用上面那条"可作废的目标"判定之前，先看 `R` 自己是不是一条**路径2的 `✨` 触发消息**——即 `R` 含 `reply` 段，且除 `reply` 外只有一个 `text` 段、文本 trim 后等于 `✨`。是的话，撤稿目标改成 **`R` 引用的那条消息**，然后上面全部既有判定（发送者是不是被观察者 / 是不是路径3格式 / 下面那条 🔥 链展开）对新目标原样再跑一遍。
...
| 层级 | 信号 | 覆盖范围 | 输掉的一方 |
|---|---|---|---|
| 0 | 💤（4.5.2 节） | 这个群这一轮的**全部**候选 | 四条路径一律不落库，但仍如实计进 `skipped`；💦 撤稿与 `lastrun` 不受影响 |
| 1 | 路径 4（🔥 链） | 链的**内容成员**（含主键） | 这些成员不再单独产生路径 1/2/3 候选 |
| 2 | 路径 3（`✨ 内容` / `✨ @某人 内容`） | 这一条消息自己 | 同一条消息的路径 1 候选被替换 |
| 3 | 路径 1 / 路径 2 | 这一条消息自己 | —— |

💤 排在最前面不是"又一条路径优先级"，它跟路径之间根本不是同一类比较：路径 1–4 回答的是"这条消息该不该、以什么正文被收录"，💤 回答的是"这个群这一轮要不要落库"，是一道更前置的闸门，它不改变任何一条候选的判定结果，只决定这批已经算好的候选写不写进 Redis。
...
管理员在窗口内发一条**只有 💤**（U+1F4A4）的消息 → **这个群**这一轮一条语录都不收录。

**「只有 💤」的精确定义**（三条同时成立，实现在 `rules.zig` 的 `sleepCommand`）：

1. `m.user_id ∈ ADMIN_QQS`。
2. `m` **不含 `reply` 段**。💤 是一条群级指令，没有作用对象；这套语法里带 `reply` 段的消息一律是"针对被引用那条"的意思（路径2的 `✨`、Pass A 的 `💦`），让 💤 也能带 reply 就是让两套语法打架——跟路径3"含 `reply` 段的一律走路径2判定"是同一条既有原则。
3. 除 `reply` 外**恰好只有一个 `text` 段**（`soleTextBesidesReply`：夹带 image / face / at / 多个 text 段一律不算），且它按空格/制表/CR/LF trim 之后**逐字节等于** `💤`。用"trim 之后等于"而不是"包含 💤"：`💤💤`、`睡了💤`、`💤 明天见` 都不是这条指令——一条能让整个群这一轮静默的开关，触发条件必须窄到不可能被闲聊误触。

**运营方的四条裁定，逐条落实**：

- **作用域只到这一个群**，不是整轮。`QQ_GROUP_IDS` 里其它群完全不受影响——`skip_collection` 是 `rules.classify` 的返回值，而 `classify` 本来就是逐群调用一次的。
- **这个群照样扫、照样发它那条七行合并转发**，全部候选如实计进 `skipped`，所以第六行读作 `Added 0 messages, skipped N messages.`。`rules.classify` 因此**不**清空 `candidates`，只多返回一个 `skip_collection` 标志，由 `scan/runner.zig` 决定不写库、只计数。诚实报出来是重点：一个安静发不出东西的群，跟一个死掉的服务，在群里必须长得不一样。
- **💦 撤稿照常执行。** 💤 的意思是"今天别加东西"，不是"忽略撤稿"。落盘顺序本来就是"作废先于收录"（4.4 节，`scanGroup` 步骤 6 先于步骤 7），跳过只作用在步骤 7 上。一条被睡觉表情吞掉的撤稿，正是这个项目一直在消灭的那类静默失败。
- **`lastrun` 照常前移。** 跳过不产生任何 `Trouble`，这个群这一轮判成功，`applyLastRun` 照写。不前移的话，下一次触发会按 4.1 节从同一个起点重扫同一个窗口、看到同一条 💤、再跳过一次——一个自我持续的永久停摆，跟"作者已离群导致窗口永远卡住"（4.7 节末尾那段）是同一个形状的 bug。

**取消**：同一个窗口里，某位管理员用 💦 引用那条 💤 消息 → 这条 💤 作废，这个群这一轮照常收录。判定看的是"有没有一条管理员 💦 **直接引用**了这个 `message_id`"，**不是**看它有没有进 `revoked`：💤 那条消息的发送者是管理员，在 `OBSERVED_QQS` 配了子集且管理员不在里面时它按 4.3 节根本不是"可作废的目标"，永远进不了 `revoked`——拿 `revoked` 判会让取消功能在那种配置下静默失效。窗口里有两条 💤 而只撤掉一条时，剩下那条仍然生效：跳过是"任意一条 💤 说了算"，撤销必须逐条撤干净，方向上偏保守的那一边跟撤稿"宁可多 tombstone 也不能少"是同一个取向。
...
- **`scan/rules.zig`** —— 输入窗口内的消息数组 + 配置（被观察者、管理员集合），输出 `{revoked: []id, candidates: []Candidate, unresolved: []id, skip_collection: bool}`。表情回应数据（✨ 的 `star_ids`、🔥 的 `fire_ids`）以参数形式注入，不在此模块内发起调用。
...
- 路径 3：`✨ @某人` 后面没有正文 → 剥完为空 → 不产生候选
- 路径 3：消息以 at 开头（渲染出来是 `@…`）→ 不是路径3；前导 image 段 / 空白 text 段不影响判定
- 路径 3：`at.data.qq` 是 `"all"`（@全体成员）→ 不当作者标记，原样渲染进正文
- 💦 引用一条路径2的 `✨` 触发消息 → 一跳解析到它引用的原消息并作废；原消息是链成员时整条链一起作废
- 💦 引用的 `✨` 触发消息，其引用目标解析不出来 → 记 `unresolved`，什么都不作废
- 💦 引用的是普通引用消息（不是 `✨` 触发形状）→ 不发生一跳，行为逐字不变
- 💤：管理员发一条只有 💤 的消息 → `skip_collection` 为真，`candidates` 仍照常算出（供 `skipped` 计数）
- 💤：非管理员发 / 带 reply 段 / 夹带其它段或别的字 → 都不算
- 💤：同一窗口里的 💦 撤稿照常进 `revoked`
- 💤：管理员 💦 引用那条 💤 → 取消跳过；管理员不在观察集合里时同样生效（判的是"被 💦 直接引用"而不是 `revoked`）
- 💤：两条 💤 只撤掉一条 → 仍然跳过
