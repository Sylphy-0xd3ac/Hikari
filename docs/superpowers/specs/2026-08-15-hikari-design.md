...
- 去掉开头的 `✨` 及紧随其后的空白（可有可无）
- → 候选是 `m` 自身，**正文为剩余部分**（不含 `✨` 前缀）；普通 `✨ 内容` 的正文必须非空，光杆 `✨` 保持旧行为、不形成候选。只有下面已经识别到作者标记的 `✨ @某人` 允许空正文，并交给 4.6 节的文本空关卡诚实计入 `skipped`
...
**完整优先级：已确认的 💤 > 路径 4 > 路径 3 > 路径 1。** 路径 2 与路径 3 互斥，不参与这个顺序；同一条消息被多条路径命中时按 `message_id` 去重，只入库一次。

| 层级 | 信号 | 覆盖范围 | 输掉的一方 |
|---|---|---|---|
| 0 | 已确认的 💤（4.5.2 节） | 这个群这一轮的**全部**候选 | 四条路径一律不落库，但仍如实计进 `skipped`；💦 撤稿与 `lastrun` 不受影响 |
| 1 | 路径 4（🔥 链） | 链的**内容成员**（含主键） | 这些成员不再单独产生路径 1/2/3 候选 |
| 2 | 路径 3（`✨ 内容` / `✨ @某人 内容`） | 这一条消息自己 | 同一条消息的路径 1 候选被替换 |
| 3 | 路径 1 / 路径 2 | 这一条消息自己 | —— |

已确认的 💤 排在最前面不是"又一条路径优先级"，它跟路径之间根本不是同一类比较：路径 1–4 回答的是"这条消息该不该、以什么正文被收录"，💤 回答的是"这个群这一轮要不要落库"，是一道更前置的闸门，它不改变任何一条候选的判定结果，只决定这批已经算好的候选写不写进 Redis。单独的 💤 锚点没有任何控制效果，不参与这个优先级。
...
| `from_who` | 语录**作者**（不是固定的被观察者）的群名片。路径3用了 `✨ @某人 内容` 语法时，作者是**被 at 的那个人**，不是敲指令的管理员（4.5 节路径3）。渲染时优先取 `hikari:username:{user_id}` 的实时值；这个键缺失（导入的语录、这个人从未被某次扫描当过候选作者、或者已经离群且从未刷新成功过）才落回 hash 里冻结的快照 |
| `creator` | 路径1、2、4固定 `"Hikari"`；路径3（管理员手动 `✨ 内容` / `✨ @某人 内容`）是**那位管理员**当时的群名片/昵称——他本人在 Hitokoto 语义下就是这条语录的**添加者**，跟「这句话是谁说的」（`from_who`）是两回事，用了 at 语法时两者是不同的人 |
| `creator_uid` | 路径1、2、4固定 `0`；路径3是**那位管理员**的 QQ，不是被 at 的作者的 QQ |
| `reviewer` | 固定 `0` |
