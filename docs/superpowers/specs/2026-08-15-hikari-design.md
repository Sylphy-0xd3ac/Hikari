...
**"这一轮有件事没看清楚"会顶住 `lastrun`。** 有两种情况会让本轮的窗口必须被下一轮重新覆盖一次
（`commitAfterHold`，取全部来源里**最早**的那个时刻）：

- **表情探针失败**：被观察成员的某条消息在 4.4 节的探测里 `get_msg` 连同唯一一次重试都失败，
  它挂没挂 ✨/🔥 本轮无从得知。照常前移的话下一轮窗口从它后面开始，这条语录永远不会再被看到
  ——一次网络抖动换一条永久丢失。顶在这条消息的 `time` 上。只探 💤 的非观察成员不参与，
  理由见 4.5.2 节"核验失败 = 未确认"。
- **引用目标彻底解析不了**（`outcome.unresolved`，`unresolvedHold`）：最要命的是 💦——一条撤稿
  指令就此蒸发，一句本该下架的话继续挂在公开 API 上。丢语录只是少收一句，丢撤稿是反过来的。
  顶在**窗口里那条发出指令的消息**的 `time` 上，而不是解析不了的目标本身（它的时刻正因为解析
  不了而无从得知）。💦 的一跳形状（目标那条 ✨ 是几天前的、只在 pool 里）靠 `triggerTarget`
  反查回窗口里的那条指令。

两者都不做"重试队列"，而是顶住窗口：🔥 链的归并（4.4 节）、💨 的补丁判定、💦 的一跳都依赖这条
消息在窗口里的**上下文**，脱离上下文单独重放会改变语义。最坏情况有界且自愈：某件事永久看不清楚时
窗口起点钉在那里逐日变长，到 4.1 节的 7 天上限被 clamp 跳过并告警，届时恢复正常；这期间收录照常
进行。选这个而不是干脆判这个群失败，正是为了躲开"永远卡住"那种 bug 形状（4.7 节末尾）。

稳态窗口长度约等于「每轮顶窗口的事件数 + 1」天（事件位置在窗口内近似均匀时，最早那个落在
1/(k+1) 处），k=6 才会撞到 7 天上限。runner 在顶住窗口时打一条指名回退秒数的 warn，运维据此
判断 k；具体是哪一类，各自在发生的地方已经打过一条。

**这条机制的边界**：它救得回"落单的一条 ✨ 语录"，救不回"链里某条消息探针失败"。后者在本轮里
链已经断了，幸存的内容消息会退回路径1各自单独入库，下一轮重扫时它们已在 `hikari:index` 里、
只会计 `skipped`，链不会事后补成。要彻底解决得在探针失败时抑制整批候选的写入，代价是一次抖动
让整个群这一轮颗粒无收——权衡下来不做，这里如实记下缺口。

消息以 OB11 的 `time` 字段（Unix 秒）判定归属。**`time` 缺失或解析不了时 `onebot.parseMessage`
填 0**，而 0 必然早于任何真实窗口起点——放任它参与"这一页最老是什么时候"的判断，一条坏消息就能
让翻页在第一页认定已经翻到窗口起点、再拉一页缓冲收工，连 4.2 节那条 `history window NOT fully
covered` 警告都不会打，更早的那一段连同其中只会被看到一次的 💦 就此永久跳过，群里照报
`Successfully in Ns.`。真实的群消息不可能来自 1970 年，所以 `time <= 0` 一律当"时间戳不可用"：
既不参与选最老时间，也不参与选翻页锚点（`hasUsableTime`）。整页都不可用时两个函数都返回 null，
翻页循环既有的两个 `break` 各自带 `stop_reason` 收尾，警告照打。这类消息同时也进不了判定窗口
（`inWindow(0, …)` 恒假），所以 runner 必须留下痕迹——不留的话，现象跟"这条消息压根没发过"
一模一样。告警按**页**聚合成一行（`unusableTimeSummary`）：点名前 8 条 `message_id`、其余只报数。
不逐条打是因为一页 200 条、护栏又允许上千页，"每页都掺着坏消息但又都有好消息让翻页继续"这种
形状能刷出几十万行 warn，把真正要看的告警埋掉；而排查只需要知道"具体哪几条没了"和"是不是
成规模地没了"。
...
**"翻页停下"必须分成两类**（`pageStopReason` 返回的 `PageStop.reached_start`）：

- **正常收尾**（`reached_start = true`）：空页、或只剩锚点自己一条——再往前**没有**消息了，
  窗口是被完整覆盖过的，哪怕它的起点比这个群的第一条消息还早。
- **窗口没被完整覆盖**（`reached_start = false`）：回复不是对象、没有 `messages` 字段、
  `messages` 不是数组、整页消息一条都解析不出、锚点不再前进（NapCat 原样返回同一页）、
  翻页护栏耗尽。

护栏本身跟着**窗口跨度**走（`pageGuard`）：每 24 小时给 200 页，不足一天按一天算，上限是 7 天
回看上限对应的 1400 页（28 万条）。写死 200 页是按 24 小时窗口标定的，而顶窗口机制会让窗口一路
长到 7 天——日均 5700 条以上的群在长窗口下必然撞满护栏 → 判"覆盖不全" → 又顶住窗口 → 下一轮
窗口更长更扫不完，直到 clamp 生效才自愈，中间每一轮都白拉 4 万条消息却什么都不提交。上限不跟着
`--last` 的任意窗口一起放大：强制窗口能造出任意跨度，护栏不能跟着跑飞。

后者意味着 `[窗口起点, 这一轮拉到的最老一条)` 这一段**完全没被检查过**。改动之前这里只打一条
`history window NOT fully covered` 警告，`lastrun` 照常推到 `now`——而那条警告的正文自己就写着
"漏掉的那一段永远不会被重扫，里面的 💦 不可恢复"。警告再响也拦不住数据丢失，何况这一段从来
没被人看过，连"丢了什么"都不知道。现在改为把 `lastrun` 顶回**窗口起点**（4.1 节的顶窗口机制），
下一轮从同一个起点重新覆盖整段。顶在窗口起点而不是"这一轮拉到的最老一条"，因为没覆盖到的正是
它们之间那一段，顶在最老一条上等于把它原样再跳一次。有界性由 7 天回看上限提供：`lastrun` 不前进
时窗口起点会随 clamp 一起往前挪，所以持续覆盖不全的群最多每轮重扫 7 天，并在 clamp 生效时另有
一条明确告警。

这也是为什么"空页"必须判成 `reached_start = true`：判错的话，每个历史比窗口还短的小群都会天天
挨一条假警报，还会被这条规则永久钉在原地。
...
- **探针覆盖**：桥能成立的前提是它的 🔥 真的被探到过，而表情探测（`get_msg`）默认只盖被观察者与 💤 锚点。runner 因此额外探"**紧跟在一条已确认带 🔥 的消息之后**"的那一条，无论作者是谁；连续几座桥会一座接一座把这个条件传下去。只多探这一类而不是无差别探整个窗口，是让额外开销正比于真实的链延续尝试次数而不是群里的聊天总量（生产上一个窗口已有约 2300 次探针）。没被探到的消息一律按"没带 🔥"处理，与本节走法里"走到第一条没有 🔥 的消息就结束"同一个读法。**这条曾经缺失**：旧实现的探针循环对非观察成员直接 `continue`，`fire_ids` 只装被观察者，于是配了 `OBSERVED_QQS` 的环境里跨人桥接完全失效、链断成碎句；观察全员（生产当前配置）时 `isObserved` 恒真，掩盖了这个缺口。
- **`fire_ids` 是作者无关的**：`carriesFire` 只问"这条消息带没带 🔥"，内容成员的作者要求由 `isChainContent` 单独把关。`star_ids` 相反，只装被观察者的消息——✨ 是"这句话值得收录"的标记，非观察成员被点 ✨ 不该变成一条语录。
...
**OCR 回退**：候选先通过 tombstone / existing / chain-member 关卡；正常正文为空且目标消息至少有一个带可下载 URL 的归一化 image/mface 段时，runner 调按需启动的本机 RapidOCR。普通图片使用 `data.url`（或本身就是 HTTP(S) URL 的 `file`）；NapCat 所在机器上的裸文件名/路径不能跨机读取，直接跳过。原生 mface 缺 URL 时，`onebot.parseMessage` 按 NapCat 当前源码的 `https://gxh.vip.qq.com/club/item/parcel/item/{emoji_id前两位}/{emoji_id}/raw300.gif` 规则补出图源。

下载只允许 HTTP(S)、最多 3 次重定向、连接 5 秒/总计 15 秒，图片最多 16 MiB，并放进 mode 0600 的随机临时文件。每张图启动一个固定虚拟环境里的 RapidOCR 3.9.2 进程，使用其 PP-OCRv6 small 文字检测/中英文识别模型与 CPU ONNX Runtime 1.28.0。RapidOCR 外层 `Global.max_side_len=960` 对解码后的最长边做真正的硬限制（仅设置 detector 的 `limit_side_len` 在 3.9.2 的 `max` 分支里并不生效）；最多 200 个候选框、batch=1，ORT intra/inter-op 线程分别为 2/1。输出为结构化文本框和单字符框：低于 0.55 的文本框丢弃；单字符框的几何 gap 用于恢复明显的中文词间空格；与较高置信正文重叠的单独装饰框丢弃。每个进程由 `timeout` / `prlimit` 限制在 20 秒 CPU/墙钟、2 GiB 地址空间，模型不常驻。生产通过可选 `OCR_PYTHON_PATH` 固定解释器；完整安装下初始化只校验本地模型，模型缺失或校验失败时 RapidOCR 自身可能尝试联网恢复，因此部署验收必须先运行 `rapidocr check`。
...
| 键 | 类型 | 内容 |
|---|---|---|
| `hikari:quote:{message_id}` | HASH | 第 4.7 节全部字段，外加仅用于内部归属的 `message_id` / `group_id` |
